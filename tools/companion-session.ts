...
export interface CompanionSession {
  ...
  /** Writes one already framed record (JSON or a binary image/mesh frame) to
   * the current connection. False means the kernel buffer is full or there is
   * no connection: the caller keeps its queue and continues from `drained`. */
